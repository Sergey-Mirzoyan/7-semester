...
def encrypt(input_bytes, key):
    ...
    key_schedule = key_expansion(key) #Расширение ключа

    state = add_round_key(state, key_schedule)

    for rnd in range(1, nr):
        state = sub_bytes(state) #Подстановка, (табличнуая замена )
        state = shift_rows(state) #Циклический сдвиг влево всех строк массива данных, за исключением нулевой
        state = mix_columns(state)#умножение каждого столбца массива на фикс. полином(матричное умножение)
        state = add_round_key(state, key_schedule, rnd) #Наложение на массив данных материала ключа
...
def decrypt(cipher, key):
    ...
    rnd = nr - 1
    while rnd >= 1:
        state = shift_rows(state, inv=True) #обратный сдвиг
        state = sub_bytes(state, inv=True) #обратная подстановка
        state = add_round_key(state, key_schedule, rnd) #xor с ключом
        state = mix_columns(state, inv=True) # обратное перемешивание
